...
（今年公布第 11 週 11/19，截止 12/10 18:00）
...
- 44100/8000 = 441/80，故 L = 80、M = 441；ω_c 與增益請在 README 自行推導並說明

## FIR 低通濾波器（視窗法，Ch. 7.5）

    h[n] = sin[ω_c (n − (P−1)/2)] / [π (n − (P−1)/2)] · w[n]

- ω_c：截止頻率（由 L、M 決定，須在 README 推導）
...
- 明確給定 L、M 值；ω_c 與濾波器增益的推導列為 README 必答
- 建議說明多相（polyphase）實作以避免對零值取樣運算（選做加分）
- 繳交方式比照 HW1/HW2：個人 repo 的 `hw3/` 資料夾 + README，LMS 登錄 repo URL＋commit SHA
