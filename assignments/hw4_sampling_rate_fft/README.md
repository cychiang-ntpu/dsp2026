...
|---|---|---|
| P | 441 | 每框樣本數 |
| Q | 1025 | 低通濾波器長度（同 HW3） |
| N | 2048 | FFT 點數，x_r、h 補零至 2048 |
| H[k] | FFT{h[n]} | |

**Stop to think**：能否直接在頻域設計理想低通 H[k]（通帶 = L、阻帶 = 0）？
（教學提示：不行。brick-wall H[k] 對應的 h[n] 為週期 sinc、長度 N，不滿足 Q ≤ N − P + 1，
圓周卷積會產生時域混疊；且截斷後有 Gibbs 現象。）
